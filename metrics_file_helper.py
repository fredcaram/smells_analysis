...
def handle_raw_metrics_files():
    for suffix in file_suffixes:
        pathlist = Path(raw_metrics_folder).glob('**/*{0}.csv'.format(suffix))
        for path in pathlist:
            filepath = str(path)
            project_prefix = get_project_prefix(path.name, suffix)
            print(filepath)
            print(project_prefix)
            split_files_by_type(filepath, project_prefix, suffix)


def split_files_by_type(csv_file_path, project_prefix, suffix):
    lineNumber = 1
    newFile = True
    for line in open(csv_file_path, "r").readlines():
        ...
        if newFile:
            ...
            outHandle = open("./{0}/{1}_{2}_{3}.csv".format(metrics_folder, project_prefix, instance_type.lower(), suffix), "w")
            newFile = False

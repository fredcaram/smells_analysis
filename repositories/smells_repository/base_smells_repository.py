...
class base_smells_repository:
    ...
    def get_annotated_smells_df(self, db, project_id, smell):
    ...
    def get_smells_dataset_by_project_id(self, smell, project_id, dataset_id):
        ...
        if project is None:
            return pd.DataFrame()
        metrics_df = self.get_metrics_dataframe(project["prefix"], dataset_id, smell)
        smells_df = self.get_annotated_smells_df(db, project_id, smell)
